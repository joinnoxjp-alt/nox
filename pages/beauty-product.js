...
if (!product) {
  ...
  document.querySelector('meta[property="og:title"]').content = `${product.name}｜MIRÈIO｜NOX BEAUTY`;
  document.querySelector('meta[property="og:description"]').content = product.description || product.name;
  if (safeUrl(product.mainImage?.url)) document.querySelector('meta[property="og:image"]').content = product.mainImage.url;
  ...
  document.querySelector("[data-product-detail]").innerHTML = `
    <div class="product-intro"><div class="product-detail-media">${mainImage}</div><div class="product-detail-info"><span class="partner-badge">NOX OFFICIAL PARTNER BRAND</span><span class="eyebrow">${esc(product.shortName)}</span><h1 class="beauty-heading">${esc(product.name)}</h1><p class="product-volume">${esc(product.volume)}</p><p class="price-large">${yen(product.price)}</p><p>${esc(product.description || "")}</p><a class="beauty-button" href="${orderUrl}">購入する</a></div></div>
    ...
    </div><div class="product-trust"><div><b>MADE IN KOREA</b><p>原産国：韓国</p></div><div><b>商品情報</b><p>内容量：${esc(product.volume)}<br>JAN：${esc(product.janCode || "—")}</p></div><div><b>日本語商品表示</b><p>商品パッケージをご確認ください。</p></div><div><b>NOX公式パートナーブランド</b><p>MIRÈIO販売事業者より直接発送します。</p></div></div>${remainingMedia.map((media) => `<div class="media-frame">${mediaMarkup(media, product.name)}</div>`).join("")}<div class="product-final-cta"><p>NOX公式パートナーブランド｜MIRÈIO（ミルアジュ）</p><a class="beauty-button" href="${orderUrl}">購入する</a></div></div>`;
  document.querySelector("[data-buy]").href = orderUrl;
  document.querySelector("[data-mobile-price]").textContent = yen(product.price);
}
