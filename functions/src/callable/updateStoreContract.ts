...
import {
  createAdminAuditLogDraft
} from "../audit/adminAudit";
...
function parseInput(
  ...
  const optionCodes =
    Array.isArray(value.optionCodes)
      ? value.optionCodes.map((item) =>
          requiredText(item, 40)
        )
      : [];

  if (
    optionCodes.length > 2 ||
    new Set(optionCodes).size !==
      optionCodes.length
  ) {
    throw invalidArgument();
  }

  const planCode = enumValue(
    value.planCode,
    PLAN_CODES
  );

  let customPlan:
    ContractInput["customPlan"];
  if (planCode === "custom") {
    if (!isRecord(value.customPlan)) {
      throw invalidArgument();
    }
    const durationMonths =
      value.customPlan.durationMonths;
    const listingAmount =
      value.customPlan.listingAmount;
    if (
...
function pricingPlan(
  ...
  const raw =
    catalog.listingPlans?.[
      fieldByCode[input.planCode]
    ];
...
function pricingOptions(
  ...
  return optionCodes.map((optionCode) => {
    ...
    const raw =
      field ? catalog.options?.[field] : null;
    if (
      !isRecord(raw) ||
...
function safeContractSummary(
  ...
  return {
    ...
    contractStartAt:
      data.contractStartAt ?? null,
    contractEndAt:
      data.contractEndAt ?? null,
    totalAmount:
...
export const updateStoreContract = onCall(
  ...
  async (request) => {
    ...
    const startAt = parseTimestamp(
      input.contractStartAt
    );
    const endAt = parseTimestamp(
      input.contractEndAt
    );
    ...
    const storeReference =
      firestore.doc(
        `stores/${input.storeUid}`
      );
    const contractReference =
      firestore.doc(
        `storeContracts/${input.storeUid}`
      );
